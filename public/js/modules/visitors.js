  ...
  tbody.innerHTML = list.length ? list.map(v => `
    <tr>
      <td><div style="display:flex;align-items:center;gap:10px">
        <div class="visitor-avatar" style="width:32px;height:32px;font-size:12px;flex-shrink:0">${v.full_name[0]}</div>
        <div><div style="font-weight:600">${v.full_name}</div><div style="font-size:12px;color:var(--text-muted)">${v.company_name||'—'}</div></div>
      </div></td>
      <td>${v.tc_no||'—'}</td>
      <td>${v.reason||'—'}</td>
      <td>${v.host_name||'—'}</td>
      <td>${v.arrival_time ? formatTime(v.arrival_time) : '—'}</td>
      <td>${calcDuration(v)}</td>
      <td><span class="status-badge status-${v.status}">${statusLabel(v.status)}</span></td>
      <td>
        <div style="display:flex;gap:6px;flex-wrap:wrap">
          ${v.status==='waiting' && v.is_approved===0 ? `<button class="btn-primary" style="font-size:11px;padding:5px 10px;background:var(--green);border:none" onclick="verifyPersonnelGuest(${v.id})">✅ Onayla</button> <button class="btn-sm danger" onclick="declinePersonnelGuest(${v.id})">❌</button>` : ''}
          ${v.status==='waiting' && v.is_approved===1 ? `<button class="btn-primary" style="font-size:11px;padding:5px 10px" onclick="arrivedVisitor(${v.id})">Girdi ✓</button>` : ''}
          ${v.status==='inside' ? `<button class="btn-secondary" style="font-size:11px;padding:5px 10px" onclick="checkoutAction(${v.id})">Çıkış</button>` : ''}
...
async function showVisitorDetail(id) {
  ...
  if (!v) return;
  showModal(`👤 ${v.full_name}`, `
    <div style="display:grid;gap:12px">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
        <div><div style="font-size:11px;color:var(--text-muted);font-weight:600;text-transform:uppercase;margin-bottom:4px">TC / Kimlik</div><div>${v.tc_no||'—'}</div></div>
        <div><div style="font-size:11px;color:var(--text-muted);font-weight:600;text-transform:uppercase;margin-bottom:4px">Telefon</div><div>${v.phone||'—'}</div></div>
        <div><div style="font-size:11px;color:var(--text-muted);font-weight:600;text-transform:uppercase;margin-bottom:4px">Firma</div><div>${v.company_name||'—'}</div></div>
        <div><div style="font-size:11px;color:var(--text-muted);font-weight:600;text-transform:uppercase;margin-bottom:4px">Ziyaret Sebebi</div><div>${v.reason||'—'}</div></div>
        <div><div style="font-size:11px;color:var(--text-muted);font-weight:600;text-transform:uppercase;margin-bottom:4px">Görüşülen</div><div>${v.host_name||'—'}</div></div>
      ...
      ${v.notes ? `<div><div style="font-size:11px;color:var(--text-muted);font-weight:600;text-transform:uppercase;margin-bottom:4px">Notlar</div><div>${v.notes}</div></div>` : ''}
...
async function exportCSV() {
  ...
  a.href = `/api/logs/export?auth=${token}`;
