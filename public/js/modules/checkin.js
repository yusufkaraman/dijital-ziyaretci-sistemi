...
function buildHostSuggestionList(personnel, companyName, queryText, options) {
  ...
  const base = source.map((p) => {
    ...
    return {
      id: p.id,
      label: `${p.full_name}${titlePart}${companyPart}`,
...
async function loadCheckinPage() {
  ...
  if (companySelect) {
    companySelect.innerHTML = '<option value="">Seçiniz...</option>' +
      companyCache.map((c) => `<option value="${c.name}">${c.name}</option>`).join('');
  }

  // Preview updater
  const nameInput = document.getElementById('ci-name');
  const compInput = document.getElementById('ci-company');
  if (nameInput) nameInput.oninput = updatePreview;
  if (compInput) {
    compInput.oninput = updatePreview;
    compInput.onchange = updatePreview;
  }
  ...
  setupHostSuggestionInput({
    ...
    companyProvider: () => document.getElementById('ci-company')?.value || '',
  });
...
function updatePreview() {
  ...
  const comp = document.getElementById('ci-company').value || 'Gelinen Kurum/Firma';
...
async function loadRecentCheckins() {
  ...
  el.innerHTML = list.slice(0, 5).map(v => `
    <div class="visitor-row" style="padding:10px 0">
      <div class="visitor-avatar" style="width:28px;height:28px;font-size:11px">${v.full_name[0]}</div>
      <div class="visitor-info"><div style="font-size:13px;font-weight:500">${v.full_name}</div><div style="font-size:11px;color:var(--text-muted)">${v.arrival_time ? formatTime(v.arrival_time) : '—'}</div></div>
...
async function checkInVisitor() {
  ...
  const send_email = document.getElementById('ci-email-notif').checked;
  ...
  try {
    const v = await api.createVisitor({ 
      ...
      company_name: company || null, host_personnel_id: host_id||null, 
      reason, vehicle_plate: plate, visitor_count: count, 
      notes, send_email: send_email 
    });
    const hostNote = hostText ? ` (${hostText})` : '';
    showToast(`✅ ${name} kaydı başarıyla oluşturuldu${hostNote}.`);
...
function clearCheckinForm() {
  ['ci-name','ci-tc','ci-phone','ci-email','ci-company','ci-plate','ci-notes'].forEach(id => {
  ...
  document.getElementById('preview-company').textContent = 'Gelinen Kurum/Firma';
