...
function showDayDetail(dateStr) {
  ...
  if (!dayAppts.length) {
    ...
    list.innerHTML = dayAppts.map(function(a) {
      return '<div class="visitor-row appt-card-row" id="appt-row-' + a.id + '">' +
        renderCalendarBlock(a.planned_time) +
        '<div class="visitor-info" style="cursor:pointer;min-width:0" onclick="showAppointmentDetail(' + a.id + ')"><div class="visitor-name" style="font-size:14px">' + esc(a.visitor_name) + '</div>' +
        ...
        '<button class="btn-calendar appt-btn" data-appt-id="' + a.id + '">📅 Ekle</button>' +
        (a.status === 'planned' ? '<button class="btn-primary appt-btn js-quick-complete-appt" data-appt-id="' + a.id + '" data-appt-name="' + esc(a.visitor_name || '') + '" style="background:#10b981;border-color:#10b981">✓ Geldi Gitti</button>' : '') +
        (a.status === 'planned' ? '<button class="btn-secondary appt-btn" onclick="withButtonLock(this, function(){ return cancelAppt(' + a.id + ') })">✕ İptal</button>' : '') +
        '</div></div>';
    }).join('');
    ...
    el.innerHTML = list.map(function(v) {
      return '<div class="vitem" style="cursor:pointer" onclick="showVisitorDetail(' + v.id + ')">' +
        ...
        '<div style="display:flex;flex-direction:column;gap:8px;align-items:flex-end">' +
          ...
          (v.status === 'inside' ? '<button class="btn-checkout-red" onclick="event.stopPropagation(); withButtonLock(this, () => checkoutVisitorByPersonnel(' + v.id + '))">Çıkış</button>' : '') +
        '</div>' +
        '</div>';
    }).join('');
...
async function showAppointmentDetail(id) {
  ...
  showModal('📅 ' + esc(a.visitor_name), '<div style="display:grid;gap:12px">' +
    ...
    (a.status === 'planned' ? '<button class="btn-primary" style="background:#10b981;border-color:#10b981" onclick="quickCompleteAppointment(window._lastApptDetail.id, window._lastApptDetail.visitor_name);closeModal()">Geldi Gitti</button>' : '') +
    (a.status === 'planned' ? '<button class="btn-secondary" onclick="cancelAppt(' + a.id + ');closeModal()">İptal Et</button>' : '') +
    (a.status === 'planned' ? '<button class="btn-calendar" onclick="openGoogleCalendar(window._lastApptDetail);closeModal()">📅 Takvime Ekle</button>' : '') +
    '</div></div>');
...
function renderRoomReservationList(room) {
  ...
  if (!reservations.length) return '<div class="empty" style="padding:18px;text-align:center">Bugün rezervasyon yok.</div>';
  return reservations.map(function(res) {
    return '<div class="vitem" style="margin-bottom:8px">' +
      '<div class="vitem-info">' +
        '<div class="vitem-name">' + esc(res.title || 'Rezervasyon') + '</div>' +
        '<div class="vitem-meta">' + formatRoomTime(res.start_time) + ' - ' + formatRoomTime(res.end_time) + (res.user_name ? ' · ' + esc(res.user_name) : '') + '</div>' +
      '</div>' +
    '</div>';
  }).join('');
}
    ...
    grid.innerHTML = rooms.map(r => {
      const activeRes = (r.reservations || []).map(res => `
        <div style="font-size:12px;background:rgba(245,158,11,0.1);color:#d97706;padding:4px 8px;border-radius:4px;margin-bottom:4px;display:flex;justify-content:space-between;">
           ...
           ${u && window.vdPermissions && window.vdPermissions.canCancelReservation(u, res.user_id) ? `<button style="background:none;border:none;color:red;cursor:pointer;font-size:10px" onclick="event.stopPropagation();withButtonLock(this, () => cancelReservation(${res.id}))">İptal</button>`:''}
        </div>
      `).join('');
      ...
      <div style="display:grid;gap:14px">
        <div class="form-group">
          ...
          <select id="res-room" class="form-input" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border)">
            ...
            ${rooms.map(r => `<option value="${r.id}" ${Number(forceId) === Number(r.id) ? 'selected' : ''}>${esc(r.name)} (${esc(String(r.capacity))} Kişi)</option>`).join('')}
          </select>
        </div>
        <div class="form-group">
